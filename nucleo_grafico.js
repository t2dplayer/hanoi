...
default:
   trocar(num);
    ...
    if(movimentos>(Math.pow(2,numeross)-1)) {
	  ...
	  "MAS DÁ PRA FAZER MELHOR, VOCÊ GASTOU <b>"+movimentos+"</b> MOVIMENTOS<br>"+
	  "O IDEAL SERIA <b>"+(Math.pow(2,numeross)-1)+"</b><br>"+
	  '<button id="reini" onclick="sair()">REINICIAR</button>';
    } else{
	document.getElementById("aviso_texto").innerHTML = "PERFEITO!!! VOCÊ CONSEGUIU, PARABÉNS!!!<br>"+
	'<button id="reini" onclick="sair()">REINICIAR</button>';
